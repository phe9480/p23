...
#' @export 
#' 
simu.power.p23.new = function(nSim=10, n1 = rep(50, 4), n2 = rep(200, 2), m = c(9,9, 9, 9), 
                          orr = c(0.25, 0.3, 0.4, 0.2), rho = 0.7, dose_selection_endpoint = "ORR",
                          Lambda1 = function(t){(t/12)*as.numeric(t<= 12) + as.numeric(t > 12)}, A1 = 12,
                          Lambda2 = function(t){(t/12)*as.numeric(t<= 12) + as.numeric(t > 12)}, A2 = 12,
  ...
  #nSim=1; n1 = rep(50, 4); n2 = rep(200, 2); m = c(9,9, 9, 9); orr = c(0.25, 0.3, 0.4, 0.2); rho = 0.7; dose_selection_endpoint = "ORR"; Lambda1 = function(t){(t/12)*as.numeric(t<= 12) + as.numeric(t > 12)}; A1 = 12; Lambda2 = function(t){(t/12)*as.numeric(t<= 12) + as.numeric(t > 12)}; A2 = 12; enrollment.hold=4; DCO1 = 16; targetEvents2=c(300, 380); e1 = NULL; alpha=0.025; sf=gsDesign::sfLDOF; multiplicity.method = "dunnett"; method = "Disjoint Subjects"
  ...
  for (i in 1:nSim){
    ...
    if (K == 1) {bd.z[i] = qnorm(1-alpha)} else {
      if(o$method == "Disjoint Subjects"){
        ...
        if (i == 1){corr.zz = corr.z} #keep 1st data's correlation for checking boundary
  ...
  cum.pow=gsd.power.new(z = comb.z, bd.z=bd.z) #power irrespective of whether selection is correct.
  ...
  if (max.m >= m[length(m)]) {
    ...
    if (sum(s %in% best.dose) > 0) {
      ...
      #conditional power: chance of success when the dose selection is correct.
      CP = gsd.power.new(z = correct.comb.z, bd.z=correct.bd.z)
      generalized.pow= CP * length(correct.selection) / nSim
